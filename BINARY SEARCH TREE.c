...
void insert()
{
    ...
    else
    {
        current = root;
        parent = NULL;
        // traverse until insert position is found...
...
void deleteKey()
{
    ...
    // Delete Key : case 1 : Node to be deleted is a leaf node
    ...
    if(current->leftchild == NULL)
        ...
        q = current->leftchild;
    //delete the key case 3: Both subtrees exist, so find inorder successor and attach the nodes properly
    else
    {
    ...
    suc = suc->leftchild;
    //attach left of node to be deleted to left of successor node to be deleted
    suc->leftchild = current->leftchild;
    //Obtain right subtree of node to be deleted
    q = current->rightchild;
    }
    //attaching to parent node : case 1: if parent doesnt exist
    if(parent == NULL)
    {
        root = q;
        return;
